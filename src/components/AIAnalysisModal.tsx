import { X, Droplets, Zap, Brain, Cpu } from 'lucide-react';
...
export default function AIAnalysisModal({ open, onClose, technicalData, imageUrl, wasteType, grade }: AIAnalysisModalProps) {
  ...
  // Use technicalData fields, fallback to top-level props
  const displayWasteType = technicalData?.waste_type || wasteType;
  const displayGrade = technicalData?.quality_grade || grade;
  const displayMoisture = technicalData?.moisture_content;
  const displayCalorific = technicalData?.calorific_value;
  const displayConfidence = technicalData?.confidence;
  const hasData = !!(displayWasteType || displayGrade || displayMoisture || displayCalorific);
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-800 rounded-xl p-3">
                  ...
                  <p className="text-white font-bold text-sm">{displayWasteType || '-'}</p>
                ...
                <div className="bg-gray-800 rounded-xl p-3">
                  ...
                  <span className={`px-3 py-1.5 rounded-full text-sm font-bold ${getGradeColor(displayGrade)}`}>
                    Grade {displayGrade || '?'}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-800 rounded-xl p-3 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-white font-bold text-sm">{displayMoisture || '-'}</p>
                  </div>
                </div>
                <div className="bg-gray-800 rounded-xl p-3 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-white font-bold text-sm">{displayCalorific || '-'}</p>
                  </div>
                </div>
              </div>
              ...
              {displayConfidence != null && (
                <div className="bg-gray-800 rounded-xl p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-emerald-400 font-bold">{displayConfidence}%</span>
                  </div>
                  <div className="w-full h-2 bg-gray-700 rounded-full overflow-hidden">
                    <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${displayConfidence}%` }} />
                  </div>
                </div>
              )}
